...
namespace incrementally.Services
{
  public class CosmosDbService
  {
      ...
      List<string> containerNames)
    {
      ...
      {
        this._containers.Add(containerName, dbClient.GetContainer(databaseName, containerName));
      }
    }
    
    public async Task AddRecordingAsync(RecordingEntry recordingEntry, RecordingMetadata recordingMetadata)
    {
      await this._containers["recordings"].CreateItemAsync<RecordingEntry>(recordingEntry, new PartitionKey(recordingEntry.Id));
      await this._containers["recording_metadata"].CreateItemAsync<RecordingMetadata>(recordingMetadata, new PartitionKey(recordingEntry.Id));
    }
